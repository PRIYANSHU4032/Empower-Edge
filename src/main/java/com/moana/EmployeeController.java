...
public class EmployeeController {
	...
	public String login(@RequestParam String eid, @RequestParam String password, Model m) {
		Employees ad = es.findById(eid).orElse(new Employees("","","", "", "", ""));
	
		if(es.existsByEidAndPassword(eid,password)) {
			m.addAttribute("res",ad.getEname() );
			m.addAttribute("task", ad.getTask());
			System.out.println(ad.getEname());
			return"EmpLog";
